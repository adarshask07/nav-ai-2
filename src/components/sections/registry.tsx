...
const TheRegistry: React.FC = () => {
  ...
  return (
    <div className="w-full h-full md:h-[600px] max-h-[600px] bg-[#0B1121] rounded-2xl border border-white/10 shadow-2xl flex flex-col overflow-hidden relative group isolate scale-[0.85] md:scale-100 origin-top">

      <div className="p-4 md:p-6 border-b border-white/10 bg-[#020617] z-20 relative">
        <div className="flex justify-between items-start mb-4 md:mb-6">
          <div>
            <h3 className="text-lg md:text-2xl font-display font-bold text-white mb-1">The Centralized AI Refinery</h3>
            <div className="text-[10px] md:text-xs font-mono text-neutral-500 uppercase tracking-wide">
          ...
          <MagneticButton onClick={() => setIsDrawerOpen(true)} className="group">
            <div className={`flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-lg transition-all duration-300 ${isDrawerOpen ? 'bg-brand-cyan text-brand-navy shadow-[0_0_20px_#22D3EE]' : 'bg-brand-cyan/10 text-brand-cyan border border-brand-cyan/50 group-hover:bg-brand-cyan group-hover:text-brand-navy'}`}>
        ...
        <div className="w-full h-9 md:h-10 bg-white/5 rounded-lg border border-white/5 flex items-center px-3 md:px-4 mb-4 md:mb-6 transition-colors focus-within:border-white/20">
          <Search className="w-3.5 h-3.5 md:w-4 md:h-4 text-neutral-500 mr-2 md:mr-3" />
          <div className="text-xs md:text-sm text-neutral-600 font-mono">Search Workspaces, Agents, Super Agents...</div>
        </div>

        <div className="flex items-center gap-4 md:gap-6 overflow-x-auto pb-2 scrollbar-hide border-b border-white/5">
          {TABS.map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`relative whitespace-nowrap text-sm font-mono font-medium transition-colors pb-3 px-1 outline-none ${activeTab === tab ? 'text-brand-cyan' : 'text-neutral-500 hover:text-neutral-300'
                }`}
            >
              {tab}
              {activeTab === tab && (
